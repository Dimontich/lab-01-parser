...
int Json::get_int(const std::string& s)
{
    return std::stoi(s);
}

double Json::get_double(const std::string& s)
{
    return std::stod(s);
}
...
//                    throw std::exception();
                ...
                {
                    if ((int)s.substr(a + 1, b - a).find_first_not_of("0123456789.eE+-") != -1)
                        throw std::exception();
...
std::any Json::operator[](const std::string& key)
{
    if (this->is_object())
    {
        auto object = std::any_cast<std::unordered_multimap<std::string, std::any>>(json_map);
